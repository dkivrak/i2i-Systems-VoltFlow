...
import com.voltflow.core.persistence.repository.RegistrationOutboxRepository;
import com.voltflow.core.registration.HomeService;
import com.voltflow.core.registration.RegistrationPublisher;
...
@SpringBootTest(properties = {
        "spring.datasource.url=jdbc:h2:mem:voltflow_outbox;MODE=PostgreSQL;DB_CLOSE_DELAY=-1;DATABASE_TO_LOWER=TRUE",
        "voltflow.registration-outbox.retry-interval-ms=3600000",
        "voltflow.registration-outbox.startup-delay-ms=3600000",
        "voltflow.registration-outbox.acknowledgement-timeout-ms=5000",
        "voltflow.registration-outbox.initial-backoff-ms=100",
        "voltflow.registration-outbox.maximum-backoff-ms=500"
})
@ActiveProfiles("test")
...
class RegistrationOutboxIntegrationTest {
    ...
    void committedRegistrationStoresStablePayloadAndAcknowledgementMarksPublished() throws Exception {
        ...
        assertThat(invocation.topic()).isEqualTo("voltflow.asset-registration");
