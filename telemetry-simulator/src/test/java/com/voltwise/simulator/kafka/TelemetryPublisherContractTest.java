...
class TelemetryPublisherContractTest {
    ...
    void publishesPlainJsonWithStableKeyAndCompleteEventEnvelope() throws Exception {
        ...
        when(kafkaTemplate.send(eq("voltwise.telemetry"), eq("1"), org.mockito.ArgumentMatchers.anyString()))
                .thenReturn(pending);
        ...
        verify(kafkaTemplate).send(eq("voltwise.telemetry"), eq("1"), payload.capture());
