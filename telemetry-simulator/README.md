# VoltFlow telemetry simulator

The simulator is an autonomous Spring Boot process. It learns homes and
appliances only by consuming `voltflow.asset-registration`, keeps state in
memory, generates one reading per registered appliance per scheduler cycle, and
publishes UTF-8 JSON to `voltflow.telemetry`. It has no backend, PostgreSQL, or
...
`voltflow.asset-registration.dlt`. The default unique consumer group replays the
